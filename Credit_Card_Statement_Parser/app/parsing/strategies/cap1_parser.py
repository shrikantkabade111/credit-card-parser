# app/parsing/strategies/cap1_parser.py
...
class CapitalOneParser(BaseParser):
    """Advanced Capital One statement parser."""
    ...
    FIELD_CONFIG = {
        "statement_end_date": {
            "patterns": [
                r"Statement closing date[:\s]+([\w\s,]+\d{4})",
                r"Closing Date[:\s]+([\w\s,]+\d{4})",
                r"Statement (?:end|close)[:\s]+([\w\s,]+\d{4})"
            ],
            "keywords": ["statement closing date", "closing date", "statement end"],
            "table_keys": ["Statement Closing Date", "Closing Date"]
        },
        "payment_due_date": {
            "patterns": [
        ...
        "min_payment_due": {
            ...
            "keywords": ["minimum payment due", "minimum payment"],
            "table_keys": ["Minimum Payment Due", "Minimum Payment"]
        },
        "card_last_4_digits": {
            "patterns": [
                r"Credit card ending in\s+(\d{4})",
                r"Card (?:Number|Ending)[:\s]+.*?(\d{4})",
                r"Account Ending[:\s\-]+(\d{4})"
            ],
            "keywords": ["credit card ending in", "card ending", "account ending"],
            "table_keys": ["Card Number"]
        }
    }
    ...
    def parse(self) -> ExtractedData:
        ...
        for field_name, config in self.FIELD_CONFIG.items():
            ...
            if value:
                logger.info(f"[Capital One] Extracted {field_name}: {value} (confidence: {confidence:.2f})")
            else:
                logger.warning(f"[Capital One] Failed to extract {field_name}")
    ...
    def _validate_extracted_data(self) -> None:
        if self.extracted_data.payment_due_date and self.extracted_data.statement_end_date:
            if self.extracted_data.payment_due_date <= self.extracted_data.statement_end_date:
                logger.warning("[Capital One] Payment due date <= statement end date")
        if self.extracted_data.min_payment_due and self.extracted_data.total_balance:
            if self.extracted_data.min_payment_due > self.extracted_data.total_balance:
                logger.warning("[Capital One] Minimum payment > total balance")